...
public abstract class KeyboardHandlerMixin {
    ...
    private static boolean startPickKeyDragging(Screen screen, KeyEvent event, Operation<Boolean> original,
                                                @Local(name = "action", argsOnly = true) int action) {
        ...
        if (screen instanceof AbstractContainerScreen<?> containerScreen
            ...
            if (KeybindBugFixes.minecraft.options.keyPickItem.matches(event)
                ...
                if (containerScreen.getMenu().getCarried().isEmpty()) {
                    accessor.invokeSlotClicked(slot, slot.index, 0, ContainerInput.CLONE);
    ...
    private static boolean stopPickKeyDragging(Screen screen, KeyEvent event, Operation<Boolean> original) {
        ...
        if (screen instanceof AbstractContainerScreen<?> containerScreen
            ...
            if (accessor.isIsQuickCrafting() && !accessor.getQuickCraftSlots().isEmpty()) {
                accessor.invokeSlotClicked(null, -999, AbstractContainerMenu.getQuickcraftMask(0, accessor.getQuickCraftingType()), ContainerInput.QUICK_CRAFT);

                for (Slot dragSlot : accessor.getQuickCraftSlots()) {
                    accessor.invokeSlotClicked(dragSlot, dragSlot.index, AbstractContainerMenu.getQuickcraftMask(1, accessor.getQuickCraftingType()), ContainerInput.QUICK_CRAFT);
                }

                accessor.invokeSlotClicked(null, -999, AbstractContainerMenu.getQuickcraftMask(2, accessor.getQuickCraftingType()), ContainerInput.QUICK_CRAFT);
            }
